        ...
        for name in self.model_names:
            if isinstance(name, str):
                if path is None:
                    ...
                    load_path = os.path.join(self.save_dir, load_filename)
                    if not os.path.exists(load_path):
                        raise ValueError('checkpoint not found:'+load_path)
                else:
                    load_path=path
                net = getattr(self, 'net' + name)
                print('loading the model from %s' % load_path)
                ...
                checkpoint = torch.load(load_path, map_location=str(self.device))
                # checkpoint contains keys other than 'net', e.g. 'optimizer'
                ...
                try: # assume state_dict has "module" prefix 
                    print('state dict is a DataParallel object')
                    if isinstance(net, torch.nn.DataParallel):
                        net.load_state_dict(state_dict)
                    else:
                        net = torch.nn.DataParallel(net) # temporarily add a nn.DataParallel
                        net.load_state_dict(state_dict)
                        net = net.module
                except:
